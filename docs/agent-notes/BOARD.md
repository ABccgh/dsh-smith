...
## 2026-09-24 —— 短剧这一线收尾：清理、整理、上传（D-112 … D-119）
...
| 项 | 读数 |
| --- | --- |
| 本地 → 远端 | 本节写作时：本地 HEAD `7c9327d`；远端 tip **`f776c18`**，tree **相等**、**72/72** blobs 双向零差异、无重复路径段。⚠️ **这两个 SHA 每推一次就前进一格** —— 判据始终是 **tree 相等**，不是 SHA 相等 |
...
**GitHub App 切换：已评估并❌否决（D-118）—— 本轮唯一一个「决定不做什么」的结论。**
装在**个人账号**上的 App 无论如何授权都拿不到 `create_repository` / `get_me` / 4 个 gist 工具
（这些端点的 `enabledForGitHubApps` 为 `false`，逐条实测），而 **PAT 与 App 互斥且直接拒绝启动**（不是警告）
⇒ 只能切换、无法并存。用户 2026-09-24 看过代价后选择**保持现状**：继续用细粒度 PAT，
`mcp-github` 行与 `launch.mjs` **一个字节不动**，`~/.dsh/**` 未被触碰。
⚠️ **技术上没有障碍**（安装令牌会自动续期，`ReuseTokenSource` + 每请求咨询 provider）；
不切的理由是**代价**，不是可行性 —— 别把本条读成「App 不能用」。

**十个仓库已全部对齐（D-119）：10 → 9 个，剩下的九个**与各自本地源**逐一实测一致**（8 个 tree 逐字节相等，
`cn-dejure-conquest` 按 blob SHA 逐文件相同）。本轮推了三个：`ai-video-workbench`（先提交 7 个未提交文件，
再推 2 个）、`dsh-desktop`（2 个）、`dsh-duanju-script`（**空仓库** → 引导 → `-Force` 推 8 个）。
**删掉了 `ABccgh/dsh-inbox`**（本地无源；删除前它是那份实现的最后拷贝，
已留档 `D:\dsh-inbox-retired-20260924.tar.gz`，13 个文件，34.5 KB）。
⚠️ 一条踩过两次的陷阱：`push-api-ref.ps1` 与校验脚本都从 **`$PWD`** 判断「哪个仓库」，
**调用时必须把 `workdir` 设成被操作的仓库** —— 否则报错长得像「推送失败」，其实只是跑在了别的仓库上。

> ## ✅ 交接已履行（09-21 深夜写下 → **09-22 19:21 完成**）—— 下面这份是当时的计划与依据，保留作记录
>
> **结果：两个仓库都已上传并独立验证。** `dsh-smith` 远端 tip `83d6767`、tree `5ddbdd4d…`
> （**60/60** blobs 双向零差异）；`ai-video-workbench` 远端 tip
> `480f4a984d3a5b17ba0809c58fd5af7f5e193c20`、tree `b8e8c1e3…`（**327/327** blobs 双向零差异，
> 无重复路径段，`INVENTORY.md` 与 `platforms/_inbox/*` 抽验在场），且**21 个提交的 SHA 与本地逐字相同**。
> 两个推送都是 `force=False` 的快进。**九个位置全部闭合**；唯一剩下的仍是「四位专家的逐行贡献」。
> 逐条读数与两条超预期的发现见 **D-110**。
>
> **同日九处快照（2026-09-22 19:23:01，一次性重取，判据同上）：**
>
> | repo | tree 相等 | blobs 远端/本地 | tip | 差异 | 重复段 |
> | --- | --- | --- | --- | --- | --- |
> | `dsh-smith` | ✓ | 60/60 | `3555f0d` | 0 | 无 |
> | `ai-video-workbench` | ✓ | 327/327 | `480f4a9` | 0 | 无 |
> | `dsh-desktop` | ✓ | 43/43 | `deb8d3d` | 0 | 无 |
> | `cn-dejure-conquest` | ✓ | 5/5 | `b1997fb` | 0 | 无 |
> | `dsh-account-balance` | ✓ | 7/7 | `e3d9a98` | 0 | 无 |
> | `dsh-agent-memory` | ✓ | 5/5 | `19ead1d` | 0 | 无 |
> | `dsh-ima-kb` | ✓ | 9/9 | `53a3f66` | 0 | 无 |
> | `dsh-ck3-modcheck` | ✓ | 6/6 | `7f5c1e6` | 0 | 无 |
> | `dsh-inbox` | ✓ | 13/13 | `a98ecb4` | 0 | 无 |
>
> 几个位置的 remote tip 与本地 SHA 不同（`dsh-smith`、`dsh-desktop`、`dsh-agent-memory`）而
> **tree 相等** —— 这正是「判据是 tree，不是 SHA」那条通则的日常形态。
>
...
**只有 2 个有** —— 本仓库的 11 项未提交，以及 `D:\AIVideo` 领先远端 **20 个提交**（随后因本轮订正变 21）。
另外 7 个自 D-97/D-98 那次 9/9 IDENTICAL 复核以来本地 tip 未动，**但本轮仍然逐个重取远端读数复核**，
不沿用「本地未动 ⇒ 仍然一致」这条推断。

**本仓库的「清理」不是删垃圾**：`git status --porcelain --untracked-files=all` 恰好就是这次的工作，
`tmp/` 空且被锚定的 `/tmp/` 规则忽略，`tools/ck3wiki/data/` 不存在，无 `node_modules`。
所以做的是**整理表述**：`bin/drift-check.mjs`、`docs/dsh-forge.md`、`docs/agent-notes/PROJECT.md`
三处说「两个/三个预设」的陈旧计数，以及 `package.json` 的 `0.2.0 → 0.3.0`。

**推送路线与判据。** `git push` 在本机不通（TLS 层），走 `bin/push-api-ref.ps1`。`-Base` 的合法性
**先证 tree 再推**：远端 `a54442a` 的 **commit 对象**报出的 `tree.sha` 与本地 `753be6b` 的 tree 逐字相同
（`8ab8f9a9160f…`），且 49 条路径→blob 双向零差异。**成功不看脚本报告**，看三条：远端 tree == 本地 tree、
路径集合双向零差异、远端 `dsh-duanju` 路径数从 **0** 变成 **10**。（一条工具事实：`GET /git/trees/{commit_sha}`
**回显请求里的 sha**，不是 tree 的身份 —— 权威值只能从 commit 对象取。）

**唯一仍未验证的**：`dsh-duanju` 四位专家的**逐行贡献**（真的委派一次、看子进程跑起来）。

---

## 2026-09-21 —— 短剧智能体 `dsh-duanju`：**已交付、静态通过、挂载已实测**（D-99 / D-102 / D-103）

**结论先说：本仓库现在有四个 preset。** 新增的 `dsh-duanju`（短剧工坊 · DSH Duanju）做竖屏短剧的全流程，
但它被**用户自己的平台评估闸门**切成两段 —— 智能体交付剧本后**结束回合**，用户在**有戏AI** 上评估、
把读数带回来、说「满意」之后，才写分镜表。这条来自用户的原话，也是这个人格的主轴。

**为什么止步于平台边界（实测，不是推断）：** 有戏AI 没有 CLI、没有公开 API，`D:\AIVideo` 全树
**0 个凭证文件**；它的私有 app API 每一个调用都要从已登录浏览器的 `localStorage` 读 `token`。
所以登录、剧本粘贴、分镜表导入、生成、发起评估、发布这**六步是用户做的**，
本 preset **没有任何插件行**，也**没有任何配置键写着 token**。

| 项 | 读数 |
| --- | --- |
| 技能 lint | **8/8 clean** |
| 静态预检 | `preflight --preset dsh-duanju` → `rows: 28`，**`validated: 17   skipped: 9   failed: 0`** |
| 与出厂上游差异 | 共享 22 行（5 行有差异）、本 preset 独有 6 行、上游独有 9 行 —— 差异逐条即设计 |
| 打包面 | `check-pack` → **PACK OK**，`dsh-duanju` **10/10 文件**、**8 个技能**在 tarball 里 |
| 安装一致性 | `agent.cordis.yml` SHA256 两侧相同（`BA818BC0…23DB3`，复核后的当前值） |
| CI inventory | 本地复跑 → **inventory matches**（declared 与 on disk 都是四个） |
| **挂载** | **`mounted OK`** —— 2026-09-21 在一个出厂 `cordis` 会话里实测；25 条目、**24/24 enabled 行 `fiberState: 2`（`ACTIVE`）**、`tool-bash` 无 fiber（D-102） |
| **工具到达模型** | **收到一次用户转述的读数**：`expert_script`/`expert_board`/`expert_verifier`/`expert_chronicler`/`subagent_fork` 在表内，`workflow`/`ralph`/`subagent`/`tool-goal` 缺席（即验收判别键）—— 本机无机器可读台账，按转述记名。**它证明可达性，不证明逐行贡献**。`ACTIVE` 的边界：它**排除**「尝试注册并抛错」（→ `FAILED(3)`），但**不蕴含**「真的注册了」—— `register` 在一个**有条件的闭包** `mount(provider)` 里（`dsh-tool-subagent/lib/index.js:398`，调用点 `:566`/`:574`，缺席 `:575`、移除 `:570`），也**不蕴含** scope 可见性（`:579`）。见 **D-105**（它推翻了 D-104 第 2 条） |

**交付后做了一次对抗性复核，9 条发现全部成立并已修掉（D-100）。** 两条要紧的：`hook-ladder` 里那张
《金枝》付费点表**与它自称的出处矛盾**（`story.json` 的 `paywall` 实在 ep2/5/8/11，而技能写 3→4/5→6/8→9/11→12，
三段描述在文件里根本不存在），同一段还把**已被 D-21 推翻**的「前三集给足谁该付出代价」当要求；
以及**退出码 `4` 从来发不出来**（`process.exit(` 在整个 `script-gate.mjs` 里一次都不出现），
而它被重复在三处 preset 文本里。**组合面被复核为「未攻破」**：逐包读了 20 个包的声明，
只有三个包发布服务、且各在自己的 realm 内；那个空的 `workflowEngine` realm 追到实现层确认惰性。
修完重跑：lint 8/8（仓库与安装副本）、`preflight 17/9/0`、`PACK OK`、重装后 SHA256 逐字节相同（`BA818BC0…23DB3`）。

**另一条通用事实（D-101）：** 对整个 `@deepseek-ai` 目录做搜索**会以 exit 2 失败**并点名四个悬空 junction
（`dsh-tool-subagent-report` 是其中之一）—— **那是「没跑」，不是「没有」**。做缺失断言要先缩范围并配对照串。

**顺带修正的两条工作区记录（不是本仓库的，但值得知道）：** `reviews/full_script_review` 在
`D:\AIVideo` 被当「评分接口」引用了 4 处，而它在平台前端产物里**只作为文件路径子串**出现；
真实路径是 `POST /api/drama/drama-creation/{id}/review/start`（无请求体）。以及平台发布闸门是**六条**
（仓库只记录了三条），另有**发布表单自己的五条**。这两条已写进新 preset 的技能，
但**属于 `D:\AIVideo` 自己的记录层** —— 要不要去那边订正它的 `PROJECT.md`/`DECISIONS.md`，等用户决定；
我没有跨仓库写。

**还需要人做的：**

1. ~~跑一次挂载检查~~ —— **已完成（2026-09-21）**：`standingKeyFor('dsh-duanju')` → `mounted OK`。
   **它是在一个出厂 `cordis` 会话里跑的，不是本会话** —— 本会话的工具表里没有 `cordis_*`。
   归属与可重复性读法的更正见 **D-103**（`standingKeyFor` 每次调用都重取 `agent.cordis.yml` 的
   `mtimeMs+size` 戳：**改过组合后重跑 = 真正的重新挂载**；**只改技能正文时戳不变、挂载被复用**）。
2. ~~开一个 `dsh-duanju` 新会话看工具表~~ —— **已按转述记名（D-104）**：四个具名专家 ＋ `subagent_fork`
   在表内，`workflow`/`ralph`/`subagent`/`tool-goal` 缺席。**本机没有那个会话的机器可读台账**，
   所以这是**转述**不是实测（会话文件里装不下这个判据，见 D-104 第 5 条）。
   **仍未验证的只剩「逐行贡献」**：真的委派一次、看子进程跑起来。

**新进来的一份材料（D-107）：** 用户给了《有戏AI 托管发行及超创扶持计划》PDF（3 页，4,315,143 B，
sha256 `d27a7eea…`）。读出的东西已进 `youxi-platform` 技能的**带出处引文**：**平台对同一部作品做两次不同的评估**
（剧本评估 vs 发行评估；后者四维度 = 内容质量／制作完成度／题材适配度／**版权清晰度**）、
托管发行五步、**70% / 30%** 分成、12+ 渠道名单（海外 TikTok·YouTube；国内 抖音红果·腾讯视频号·火龙漫剧·快手）、
超创「**最高100%算力支持**」。**原始 PDF 没有落进本仓库**（证据是附件路径，本机无第二份副本），
**也没有写进 `D:\AIVideo\platforms\youxi\`** —— 那是另一个仓库，而它的 README 正是这类材料的指定落点；
要不要归档由用户定。

**刻意留白：** `D:\AIVideo` 里**没有**写任何文件，包括那份会把工作区规则层建起来的 `AGENTS.md`/`DRAMA.md`
（`agent-instructions` 已把 `.git` 与它们列为项目根标记，用户建了就会自动被读到）。以及两处 Phase 2：
宿主平面插件、把共享手艺技能迁到 `D:\AIVideo\.dsh\skills` 只存一份。
