...
同一个仓库里现在有**四个**预设。本节只比其中关系最近的两个 —— `dsh-smith` 与 `dsh-forge` 同属「造东西」这一侧，而 `dsh-ck3-mod` 与 `dsh-duanju` 各自是一条专业产品线；**四个预设的完整对照在** [`README.md`](../README.md) 的「该用哪个」，本文件不再复制一份（同一批事实的第二份副本是本仓库记录最多的那类损失）。
